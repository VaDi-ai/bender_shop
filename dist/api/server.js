...
function startApiServer(bot) {
    ...
    let indexHtml = null;
    try {
        indexHtml = fs_1.default.readFileSync(WEBAPP_PATH);
    }
    catch {
    ...
    // ── POST /api/orders ───────────────────────────────────────────────────────
    app.post('/api/orders', requireTelegramAuth, async (req, res, next) => {
            ...
            const order = await prisma_1.prisma.$transaction(async (tx) => {
                ...
                for (const item of items) {
                    ...
                    if (item.price !== undefined && Number(item.price) !== actualPrice) {
                        ...
                        void (0, security_log_1.logSecurityEvent)('price_manipulation_attempt', {
                            ip: req.ip,
                            telegramId,
