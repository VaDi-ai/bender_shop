...
function humanizeApiError(e) {
    ...
    if (status === 401)
        return '🔑 Неверный ключ API. Проверьте что скопировали ключ полностью.';
    if (status === 403)
        return '🚫 Доступ запрещён. Возможно ключ заблокирован или нет прав.';
    if (status === 429)
        return '⏳ Превышен лимит запросов. Подождите немного и попробуйте снова.';
    if (status === 402)
        return '💳 Недостаточно средств на балансе OpenRouter. Пополните счёт.';
    if (status === 500)
        return '🔧 Ошибка на стороне OpenRouter. Попробуйте через несколько минут.';
    if (status === 503)
        return '🔧 OpenRouter временно недоступен. Попробуйте позже.';
