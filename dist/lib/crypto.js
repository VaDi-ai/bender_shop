...
exports.getEncryptedKeyVersion = getEncryptedKeyVersion;
...
function getKeys() {
    ...
    for (let v = 1; v <= 10; v++) {
        ...
        if (val) {
            if (!/^[0-9a-fA-F]{64}$/.test(val))
                throw new Error(`ENCRYPTION_KEY_V${v} must be 64-char hex`);
    ...
    if (Object.keys(keys).length === 0 && process.env.ENCRYPTION_KEY) {
        ...
        if (!/^[0-9a-fA-F]{64}$/.test(legacyVal))
            throw new Error('ENCRYPTION_KEY must be 64-char hex');
...
function getEncryptedKeyVersion(value) {
    const m = value.match(/^v(\d+):/);
    if (m)
        return parseInt(m[1], 10);
    if (/^[0-9a-f]{24}:[0-9a-f]+:[0-9a-f]{32}$/.test(value))
        return 1;
    return null;
}
