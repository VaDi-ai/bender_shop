...
- `scripts/match-photos-to-sheets.ts` → колонка **Фото (Q)** в Sheets. Путь к файлам может быть родительским каталогом: скрипт **рекурсивно** обходит подпапки (`Apple Stock (Обработка)/`, `Samsung Stock/` …), для матчинга и URL использует **basename** (как в `/photos/<file>.webp`). Флаг **`--clear-photos`** перед матчем обнуляет «Фото» у всех строк (полное обновление каталога; без него новые URL **дописываются** к старым через запятую). Дубликат basename в двух подпапках попадёт в лог и будет пропущен. Дополнительно: если ключ `brand|family|цвет|размер` расходится, используется **сравнение полного названия строки («Название модели») с текстом имени файла** (Dice + подстрока), когда файл назван почти как товар в таблице.
